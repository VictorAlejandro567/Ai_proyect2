...
class VacuumEnvironment:
    ...
    def __init__(
        ...
        self.action_handlers = action_handlers or {
            ...
            Action.UP: self._handle_up,
            Action.DOWN: self._handle_down,
    ...
    def _handle_left(self, _: "VacuumEnvironment") -> None:
        ...
        if len(neighbors) > 0 and neighbors[0] is not None:
            self.state.agent_location = neighbors[0]

    def _handle_right(self, _: "VacuumEnvironment") -> None:
        neighbors = self.state.adjacency.get(self.state.agent_location, [])
        if len(neighbors) > 0 and neighbors[1] is not None:
            self.state.agent_location = neighbors[1]
    
    def _handle_up(self, _: "VacuumEnvironment") -> None:
        neighbors = self.state.adjacency.get(self.state.agent_location, [])
        if len(neighbors) > 0 and neighbors[2] is not None:
            self.state.agent_location = neighbors[2]
    
    def _handle_down(self, _: "VacuumEnvironment") -> None:
        neighbors = self.state.adjacency.get(self.state.agent_location, [])
        if len(neighbors) > 0 and neighbors[3] is not None:
            self.state.agent_location = neighbors[3]
...
class RandomAgent:
    ...
    def __call__(self, percept: Percept) -> str:
        return self.rng.choice([Action.SUCK, Action.LEFT, Action.RIGHT, Action.UP, Action.DOWN, Action.NOOP])
...
def build_four_location_world(
    ...
    adjacency = {"A": [None, "B",None, "C"], "B": ["A", None , None, "D"], "C": [None, "D", "A", None], "D": ["C", None ,"B", None]}  # A <-> B
    dirt = {"A": dirt_A, "B": dirt_B, "C": dirt_C, "D": dirt_D}
...
def build_line_world(n: int, dirt_prob: float = 0.5, start_index: int = 0, seed: int = 0) -> VacuumState:
    ...
    for i, loc in enumerate(locations):
        neighbors = []
        if i - 1 >= 0 and i not in (2, 0):
            neighbors.append(locations[i - 1])
        if i + 1 < n and i not in (1, 3):
            neighbors.append(locations[i + 1])
        adjacency[loc] = neighbors
        if i - 2 >= 0:
            neighbors.append(locations[i -2])
        if i + 2 < n: 
                neighbors.append(locations[i + 2])
...
def main() -> None:
    ...
    state = build_four_location_world(dirt_A=True, dirt_B=False, dirt_C=False, dirt_D=False, start="A")
    ...
    def run_trials(agent_factory: Callable[[], AgentProgram], trials: int = 100, steps: int = 30):
        ...
        for t in range(trials):
            ...
            state_t = build_four_location_world(dirt_A=dirtA, dirt_B=dirtB, start=random.choice(["A", "B", "C", "D"]))
